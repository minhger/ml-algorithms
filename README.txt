...
-----------------------------------------------------------------------------------------------------------------
To start the tool open the application visualizer.exe.

Clustering
For clustering, points can be inserted into the drawing area in one or more colors . 
The parameter values / methods required for the algorithm must then be entered. 
The points can then be grouped by clicking on the algorithm. The new color filling of the points 
indicates the group membership determined by the algorithm.

Classification
Training data is inserted into the drawing area in the same way as in cluster analysis. 
Different colors must be used to identify different classes: points of a color belong to a class. 
To classify test data, points must be inserted by clicking the test data check button. The parameter 
values required for the algorithm must then be entered. The test data can then be classified by clicking 
on the algorithm. The new color filling of the points shows the class to which the algorithm belongs.

Parameters and linkages 
    - k-means algorithm: number of desired clusters
    - Hierarchical cluster analysis: linkage, number of desired clusters
    - k-nearest neighbour procedure: number of neighbours

Downloaded data points will be saved as CSV file ("data.csv") in the default download folder.
